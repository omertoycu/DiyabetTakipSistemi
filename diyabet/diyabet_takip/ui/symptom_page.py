...
class SymptomPage(QWidget):
    ...
    def save_symptom(self):
        try:
            ...
            # Belirti kaydet
            belirti_tipi = next((k for k, v in self.belirtiler.items() if v == symptom), None)
            if belirti_tipi:
                belirti_query = """
    ...
    def load_symptoms(self):
        # Semptomları ve belirtileri birleştirerek göster
        query = """
            SELECT s.tarih, s.semptom, b.notlar, s.id
            FROM semptom_takibi s
            LEFT JOIN belirtiler b ON s.hasta_id = b.hasta_id 
                AND s.tarih = b.tarih 
                AND b.belirti_tipi = CASE s.semptom
                    WHEN 'Poliüri' THEN 'poliuri'
                    WHEN 'Polifaji' THEN 'polifaji'
                    WHEN 'Polidipsi' THEN 'polidipsi'
                    WHEN 'Nöropati' THEN 'noropati'
                    WHEN 'Kilo Kaybı' THEN 'kilo_kaybi'
                    WHEN 'Yorgunluk' THEN 'yorgunluk'
                    WHEN 'Yaraların Yavaş İyileşmesi' THEN 'yara_iyilesme'
                    WHEN 'Bulanık Görme' THEN 'bulanik_gorme'
                END
            WHERE s.hasta_id = %s
