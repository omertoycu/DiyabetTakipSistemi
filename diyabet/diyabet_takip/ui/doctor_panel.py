...
class DoctorPanel(QWidget):
    ...
    def load_patients(self):
        ...
        for p in patients:
            ...
            """
            symptom = self.db.fetch_one(symptom_query, (p[4],))
            ...
            symptom_ok = (not symptom_filter or symptom_filter in symptom_val)
            ...
            if symptom_filter and symptom_filter in symptom_val:
                warnings.append(f"{p[1]} {p[2]}: Son semptom '{symptom_val}'")
        # Tabloyu doldur
        ...
        for row, (p, sugar_val, symptom_val) in enumerate(filtered):
            ...
            self.patient_table.setCellWidget(row, 5, view_btn)
