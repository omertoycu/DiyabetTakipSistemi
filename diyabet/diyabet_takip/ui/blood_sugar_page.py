...
class BloodSugarPage(QWidget):
    ...
    def init_ui(self):
        ...
        if self.user_type == 'hasta':
            ...
            self.type_combo.addItems(["Sabah", "Ogle", "İkindi", "Aksam", "Gece"])
        ...
        # Grafik gösterim butonu ve tarih seçici
    ...
    def check_measurement_time(self, measurement_type, datetime_obj):
        """Ölçüm zamanının belirlenen aralıkta olup olmadığını kontrol eder"""
        try:
            # Ölçüm tipini küçük harfe çevir ve Türkçe karakterleri düzelt
            measurement_type = measurement_type.lower()
            if measurement_type == 'i̇kindi':
                measurement_type = 'ikindi'
            
            # Datetime nesnesinden saat değerini al
            ...
            print(f"Zaman kontrolü hatası: {str(e)}")
            return False
    
    def get_insulin_recommendation(self, average):
        """Ortalama kan şekeri değerine göre insülin önerisi döndürür"""
        if average < 70:
    ...
    def calculate_average_for_measurement(self, measurements, current_type):
        """Belirli bir ölçüm tipi için ortalama hesaplar"""
        measurement_order = ['sabah', 'ogle', 'ikindi', 'aksam', 'gece']
    ...
    def update_daily_average(self, selected_date=None):
        """Günlük ortalama kan şekeri değerini hesaplar ve gösterir"""
        ...
        measurement_values = {}
        for type_ in ['sabah', 'ogle', 'ikindi', 'aksam', 'gece']:
            type_measurements = []
            for value, m_type, datetime_obj in measurements:
                if m_type.lower() == type_:
                    if isinstance(datetime_obj, str):
                        datetime_obj = datetime.strptime(datetime_obj, '%Y-%m-%d %H:%M:%S')
                    if self.check_measurement_time(type_, datetime_obj):
        ...
        # HTML formatında mesaj oluştur (scrollable kutu)
        message = """
    ...
    def save_measurement(self):
        try:
            ...
            # datetime oluştur
            try:
                # QTime'dan Python time nesnesine dönüştür
                time_obj = qtime.toPyTime()
                # datetime.time ve date'den datetime.datetime oluştur
                datetime_obj = datetime.combine(date, time_obj)
                
                # Saat kontrolü
                if not self.check_measurement_time(measurement_type, datetime_obj):
                    QMessageBox.warning(
                        self,
                        "Uyarı",
                        f"{measurement_type.capitalize()} ölçümü için önerilen saat aralığı: "
                        f"{self.measurement_times[measurement_type]['start']:02d}:00 - "
                        f"{self.measurement_times[measurement_type]['end']:02d}:00\n"
                        f"Açıklama: {self.measurement_times[measurement_type]['description']}"
                    )
                
                # Kontrol amaçlı datetime nesnesini yazdır
                print(f"Oluşturulan datetime nesnesi: {datetime_obj}")
                print(f"datetime tipi: {type(datetime_obj)}")
                
            except Exception as e:
                QMessageBox.critical(self, "Hata", f"Tarih/saat oluşturulurken hata: {str(e)}")
                return
    ...
    def load_measurements(self, selected_date=None):
        ...
        for row, measurement in enumerate(measurements):
            ...
            # Silme butonu - sadece hasta kullanıcılar için
            if self.user_type == 'hasta':
    ...
    def delete_measurement(self, measurement_id):
        # Sadece hasta kullanıcılar silebilir
        if self.user_type != 'hasta':
